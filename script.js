...
{q:"What number comes before 10?",a:["8","9","11"],c:"9"},
{q:"What number is between 6 and 8?",a:["5","7","9"],c:"7"}
]
},
...
{q:"What color is milk?",a:["White","Black","Purple"],c:"White"}
],
hard:[
{q:"What color is chocolate?",a:["Brown","Pink","Blue"],c:"Brown"},
{q:"What color is a frog?",a:["Green","Orange","Black"],c:"Green"},
{q:"What color is a carrot?",a:["Orange","Blue","Black"],c:"Orange"}
]
},
...
{q:"Which shape looks like a star?",a:["Star","Square","Circle"],c:"Star"}
],
hard:[
{q:"Which shape has 5 sides?",a:["Pentagon","Circle","Triangle"],c:"Pentagon"},
{q:"Which shape has 6 sides?",a:["Hexagon","Square","Circle"],c:"Hexagon"},
{q:"Which shape is like a heart?",a:["Heart","Triangle","Square"],c:"Heart"}
]
}
...
function loadQuestion(){
  ...
  let levelQuestions = quizData[currentTopic][difficulty];
  ...
  let data = levelQuestions[questionIndex];
  ...
  document.getElementById("quizProgress").innerText =
    "Question " + (questionIndex+1) + " / " + levelQuestions.length;
...
function checkAnswer(btn, correct){
  ...
  if(btn.innerText === correct){
    ...
    let praises = ["Great job!", "Awesome!", "Nice!", "You're amazing!"];
...
function nextQuestion(){
  ...
  if(questionIndex >= levelQuestions.length){
    ...
    if(score >= 2 && difficulty === "easy"){
      difficulty = "medium";
      speak("Level up! Medium!");
    }
    else if(score >= 4 && difficulty === "medium"){
      difficulty = "hard";
      speak("Wow! Hard level!");
    }
    else{
...
function matchVoiceAnswer(spoken){
  ...
  buttons.forEach(btn => {
    ...
    let answerText = normalizeSpeech(btn.innerText);

    let possibleWords = synonyms[answerText] || [answerText];

    possibleWords.forEach(word => {
      if(spoken.includes(word)){
        matched = true;
        checkAnswer(btn, btn.innerText);
      }
    });
...
// BETTER SPEAK FUNCTION (CLEAR FOR KIDS)
function speak(text){
  let speech = new SpeechSynthesisUtterance(text);
  speech.rate = 0.85;  // slower = clearer
  speech.pitch = 1.1;
  speech.volume = 1;
  speech.lang = "en-US";
